import cv2
import numpy as np
from shape_detect import shape_script as ts
...
def detect():
    ...
    large_triangle = ts.triangleLarge()
    small_triangle = ts.triangleSmall()
    ...
    for i in range(0,numContours):#loop for finding shape and size of the shapes.
        ...
        cmp1 = cv2.matchShapes(cnt,large_triangle,1,0.0)
        cmp2 = cv2.matchShapes(cnt,small_triangle,1,0.0)

        if (cmp1 < 0.01 or cmp2 < 0.01):
            cv2.putText(image,"TRIANGLE",(topmost[0]+20,topmost[1]),font,0.35,(0,0,0),1)
    ...
    if l > 0:
        ...
        for i in range(0,l): #appending all the elements of color,size and shape
            string  =color[i]+ "-"+ size[i]+"-" +shapes[i]+"-"
            colored_shapes.append(string)

        new_list = [] #list for storing the final output
        ...
        #writing the lists onto a csv file..
